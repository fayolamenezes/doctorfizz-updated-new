...
function Section({
  icon: Icon,
  title,
  statusTone = "default", // "good" | "warn" | "bad" | "default"
  statusText, // e.g. "All Good", "4 Errors"
  defaultOpen = true,
  children,
}) {
...
/* ---------- Result line with optional Fix action + right info icon ---------- */
...
export default function CEResearchPanel({
  /** controlled props passed by CE.ContentArea */
  query = "",
  onQueryChange,
  onStart,

  /** reacts to MetricsStrip pills; stays “basic” visuals unless switched */
  seoMode = "basic",

  /** optional metrics (kept for future tie-ins) */
  metrics,

  /** optional callback if you want to handle Fix Now */
  onFix,
}) {
  ...
  if (phase === "idle") {
    return (
      <aside className="h-full rounded-r-[18px] border-l border-[var(--border)] bg-white/70 px-7 md:px-8 py-6 flex flex-col">
        <h3 className="text-[22px] font-semibold text-[var(--text-primary)] text-center">
          Research
        </h3>
      ...
      <Section icon={TriangleAlert} title="Additional" statusTone="bad" statusText="4 Errors" defaultOpen>
        <div className="space-y-1.5">
          ...
          <ResultItem type="error" onFix={() => onFix?.("slug-shorten")}>
            ...
            <span className="font-mono text-[12px]">/best-seo-tools-2025-for-digital-marketing-guide/</span>. Shorten it.
          </ResultItem>
